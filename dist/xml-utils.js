...
export function extractHrefsFromPropfind(xmlText) {
    try {
...
export function parseCalendarQueryResponse(xmlText, baseUrl) {
    try {
        const parsed = xmlParser.parse(xmlText);
        const events = [];
        const multistatus = parsed.multistatus || parsed['multistatus'];
...
export function buildCalendarQueryXml(filters, formatDateTime) {
    let timeRangeFilter = '';
...
export function buildSyncCollectionXml(syncToken) {
...
export function parseSyncCollectionResponse(xmlText) {
    try {
        const parsed = xmlParser.parse(xmlText);
        const multistatus = parsed.multistatus || parsed['multistatus'];
        if (!multistatus) {
