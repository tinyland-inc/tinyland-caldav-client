...
import { extractHrefsFromPropfind, buildCalendarQueryXml, parseCalendarQueryResponse, buildSyncCollectionXml, parseSyncCollectionResponse, } from './xml-utils.js';
...
export class CalendarClient {
    ...
    async createEvent(event) {
        const uid = event.uid || `${Date.now()}-${Math.random().toString(36).substr(2, 9)}@stonewallunderground.com`;
    ...
    async createEventWithETag(event) {
        const uid = event.uid || `${Date.now()}-${Math.random().toString(36).substr(2, 9)}@stonewallunderground.com`;
        const icalData = generateICalData({ ...event, uid });
        const response = await fetch(`${this.baseUrl}${this.calendarPath}${uid}.ics`, {
            method: 'PUT',
    ...
    async updateEvent(uid, event) {
        ...
        if (response.status === 412) {
            ...
            throw new CalDAVConflictError('Event was modified by another process. Refresh and try again.', event.caldavEtag, currentEtag);
        }
        if (!response.ok) {
